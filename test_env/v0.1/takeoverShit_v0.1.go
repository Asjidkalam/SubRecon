...
func main() {
	...
	if len(os.Args) != 3 {
		fmt.Println("Please specify the input and output file.")
		fmt.Println("Usage:", os.Args[0], "<hostsfile>.txt", "<outfile>.txt")
		return
	}
	hostFile := os.Args[1]
	outFile := os.Args[2]
		...
		if url != "" {
			...
			for i := 0; i < len(vulnHosts); i++ {
				if vulnHosts[i] != "" {
					...
					fmt.Println("[*] Output written to takeover_result.txt")
				}
			}
			// GC
			for i := 0; i < len(vulnHosts); i++ {
